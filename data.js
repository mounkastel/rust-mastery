  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 8.3: the entry API returns `&mut V` to the live slot — one lookup for check-and-mutate is its whole point. Options (c)/(d) describe `insert`, which returns `Option<V>` (the displaced old value).',
        },
      ],
    },
  },
  ...
  // ================= Chapter 9 — Error Handling =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'Both panic on `Err`; `expect` attaches your message to the panic output (TRPL 9.3/11.1). Options (b)/(c) invent non-panicking behavior and (d) invents a restriction. Book rule of thumb: `expect` where failure means programmer error with context worth stating.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'The `?` desugar calls `From::from` on the error (TRPL 9.2/12.3 `Box<dyn Error>` acceptance). Any `E: Error + \'static` converts into the box — that is why `main() -> Result<(), Box<dyn Error>>` absorbs every fallible call. Option (d) is what beginners write before learning this.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 10 — Generics, Traits, Lifetimes =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 10.1: monomorphization stamps out a concrete copy per use — zero-cost at runtime, larger binary as the tradeoff. Option (c) describes Java erasure, Rust\'s explicit anti-model; (d) describes `dyn Trait` dispatch instead.',
        },
        {
        type: 'find_bug',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 10.2: single-param `impl Trait` desugars to exactly the generic form (both monomorphize, both static). The difference: two `impl Trait` params are independent types, while one `T` used twice forces sameness. Options (b)/(c) wrongly assign dynamism — that is `dyn Trait`.',
        },
        {
        type: 'multiple_choice',
        ...
        correct: 'a',
        explain: 'TRPL 10.2/19.3: supertraits constrain implementors, not callers — `impl Loud for X` fails unless `X: Display` too. Option (b) inverts the direction; a trait never implements things on your behalf.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 10.3 elision: rule 1 gives `x` and `y` distinct lifetimes; rules 2–3 (single input / `&self` method) do not apply, so the output has no determinate source — E0106. Fix: `fn first<\'a>(x: &\'a str, y: &\'a str) -> &\'a str`. Option (b) states a rule that does not exist.',
        },
        {
        type: 'multiple_choice',
        ...
        correct: 'a',
        explain: 'TRPL 10.3/16: `\'static` as a bound means "bounded by the end of the program" — owned types (`String`, `i32`) qualify; `&\'short str` does not. It constrains what the type may *borrow*, not how long any value lives (b).',
        },
        {
        type: 'find_bug',
        prompt: 'struct H {\n    s: &str,\n}\nfn main() {\n    let h = H { s: "hi" };\n}\n\nWhat is missing?',
  ...
  // ================= Chapter 11 — Writing Automated Tests =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 11.1: `Result`-returning tests let `?` propagate (`Err` = failure with the value attached). Option (d) denies a stable feature; `#[should_panic]` and `#[ignore]` cover the other special cases.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 12 — I/O Project =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 12.4/13: iterator adapters encode intent (`skip`, `lines`, `collect`) where indexing encodes mechanics — and every manual index is a potential out-of-bounds panic. Option (b) overclaims; the win is correctness-by-construction.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 13 — Closures and Iterators =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 13.1: closure types are unnameable anonymous structs, so `impl Trait` in return position (or `Box<dyn Fn()>`) is the only way to name "some closure". Option (d) invents syntax; (b) is false — indirection solves sizing.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`into_iter(self)` takes ownership (TRPL 13.2) — the vector is consumed so iteration can hand out owned values. Borrowing iteration is `iter()`/`iter_mut()`. Option (c) describes `iter()`; (d) invents a clone.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 14 — More About Cargo =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 14 (features): default features serve the common case; opting out trims compile time and dependency surface. Version pinning (c) is the lockfile\'s job, not features\'.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 15 — Smart Pointers =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'The `Box` itself is a plain pointer (`Sized`) sitting wherever the variable lives — usually the stack — pointing at heap memory holding the `5` (TRPL 15.1). Option (d) inverts the picture; (c) would make `Box` pointless.',
        },
        {
        type: 'predict_output',
        prompt: 'What does this print?\n\nfn main() {\n    let b = Box::new(5);\n    println!("{}", *b + 1);\n}',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 15.6: strong-count cycles (parent↔child `Rc`s) never reach zero and leak. `Weak` breaks the cycle by observing without owning (`upgrade()` to access). Option (b) describes `Arc`; (c) describes interior mutability.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 15.5: the whole point of `RefCell` is moving borrow checking to runtime — the compiler accepts what it would reject for plain references, and `borrow_mut` panics (`already borrowed`) while `a` lives. Option (b) is the key misconception: with `RefCell`, the compiler deliberately looks away.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 16 — Fearless Concurrency =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 16.1/16.4: `Rc<T>` is `!Send + !Sync` because plain-integer increments race across threads; `Arc` pays for atomic ops. Option (d) invents per-thread cloning — both pointers share one allocation.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 16.2: `mpsc` = multiple producer, single consumer — `Sender: Clone`, `Receiver: !Clone`. Option (d) works but fights the API; the book\'s pattern is cloned senders plus one receiver.',
        },
        {
        type: 'find_bug',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 16.3: poisoning is a flag, not a deadlock — `lock()` yields `Err`, and `.unwrap()` on it is what most examples use (panicking in turn). Recovery is `poison_error.into_inner()`. Option (b) confuses poisoning with deadlock; the book stresses poisoned ≠ locked-forever.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 17 — Async =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 17: `async` produces an inert future; nothing executes until an executor polls it. Option (b) confuses async with `thread::spawn`; (d) confuses it with closures.',
        },
        {
        type: 'find_bug',
  ...
  // ================= Chapter 18 — OOP Features =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 18.1: encapsulation in Rust is a module-system property, not a class property. `pub` opens precisely what the API promises; everything else hides by default. Option (b) invents a keyword.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 19 — Patterns and Matching =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 19.1: `matches!(opt, Some(_))` is boolean pattern testing — `_` discards bindings. It shines in `assert!`/`filter` positions where a full `match` would be noise. Option (b) is backwards: `matches!` deliberately binds nothing usable.',
        },
      ],
    },
  },
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 20.4 (DST): `str`/`[T]`/`dyn Trait` lack `Sized`; fat pointers (`&str` = address + length) restore knowability. Option (c) is false — `&str` routinely borrows non-heap statics; (d) denies the language\'s core string type.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 20.5: derive macros attach to structs/enums; attribute macros rewrite arbitrary `#[x]` items (arguments like `#[route(GET, path)]` allowed); function-like macros sit in call position. Option (c) denies real-world attribute args; (d) denies `sql!`-generated items.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 21 — Final Project =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 21.2/21.3: unbounded spawning is a denial-of-service vector (slowloris-style); the pool caps concurrency, and `Drop` joins workers after disconnecting the queue so in-flight work completes. Option (b) confuses throughput architecture with latency.',
        },
      ],
    },
  },
  ...
  // ================= Bonus Module =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'Lint levels (`allow`/`warn`/`deny`/`forbid`, Appendix + TRPL App D): `deny` turns diagnostics into build failures. The book\'s posture: `warn` locally, `deny` in CI. Option (b) describes `#![allow]`; levels never change what clippy *checks*, only how failures surface.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'RBE conversions: `impl From<&str> for String` plus the std blanket `impl<T, U: From<T>> Into<U> for T`. The annotation selects which `From` applies — without it, `.into()` is genuinely ambiguous (the classic inference error). Options (c)/(d) deny the annotation-driven resolution.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'Appendix C is exactly the derivability index the book maintains. Option (c) invents a module; (d) denies the closed derive-macro set. Reference discipline: when the compiler names a missing trait, the appendix confirms whether `derive` can supply it.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'RBE attributes: `all(...)` is conjunction (`any` disjoins, `not` negates). Like all of `#[cfg]`, it gates at compile time — the item does not exist in non-matching builds. Option (c) confuses `#[cfg]` with the `cfg!` macro; (d) confuses platform predicates with Cargo features.',
        },
      ],
    },
  },
