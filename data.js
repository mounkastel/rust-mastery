...
const concepts = [
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        options: [{ id: 'a', text: 'cargo build' }, { id: 'c', text: 'cargo run' }, { id: 'b', text: 'cargo check' }],
        correct: 'b',
        ...
        correct: 'b',
        explain: '`Cargo.toml` declares a compatible range (`"1.0"` means `^1.0`: `>=1.0.0, <2.0.0`); `Cargo.lock` records the exact resolved tree, and a fresh clone reproduces it bit-for-bit. The local-cache story never overrides the lockfile, and Cargo picks the newest compatible version at `update` time — never the lowest.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 2 — Guessing Game (integrative) =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'b',
        explain: '`let mut guess` keeps one type forever; `let guess = ...` creates a fresh binding that may hold a new type. The book leans on exactly this: the name is reused while the value crosses from `String` to `u32`. The skip-the-borrow-checker and no-`mut`-in-`expect` stories invent rules that do not exist.',
        },
      ],
    },
  },
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'b',
        explain: 'TRPL 3.2: debug builds include overflow checks that panic; release builds wrap with two\'s-complement arithmetic. The `const`-evaluation story confuses this with compile-time contexts, where the compiler does reject overflow. Rust never silently widens integer types.',
        },
      ],
    },
  },
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`if` used as an expression forces both arms to one type so `s` has a single static type. Fix by unifying the arms (`else { "small" }`). Assigning an `if`-expression to `let` is perfectly legal — the arms, not the binding, are the problem.',
        },
      ],
    },
  },
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'NLL (TRPL 4.2) ends a borrow\'s region at its last use — the `println!("{r}")` line. The `&mut` for `push` starts after, so the two borrows never coexist. The never-compiles story states the pre-2018 lexical rule, which is exactly what NLL replaced.',
        },
        {
        type: 'find_bug',
  ...
  // ================= Chapter 5 — Structs =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`{}` requires `Display`, which the compiler never auto-implements. TRPL 5.2 reaches for `#[derive(Debug)]` plus `{:?}` instead. The privacy story is a red herring — fields are visible in the defining module — and struct definitions take no trailing semicolon.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 6 — Enums and Pattern Matching =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'b',
        explain: 'An enum holds one variant at a time, so Rust allocates the maximum variant size plus a discriminant tag. The summed-sizes story describes a struct; the zero-space story confuses enums with zero-sized marker types.',
        },
        {
        type: 'find_bug',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'Match ergonomics (TRPL patterns chapters): matching a reference switches bindings to borrow mode automatically. Explicit `&` still compiles but is unneeded; the always-moves story would move out of borrowed content and is rejected.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`if let` exists for refutable patterns; `x = 5` always matches, making the conditional meaningless — the compiler says so (irrefutable `if let` error, TRPL 6.3). Integers match fine as patterns; the construct, not the literal, is the problem.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 7 — Packages, Crates, Modules =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 7.1/11.3: integration tests link the *library* crate; `main.rs` is a separate binary crate invisible to them. Shared logic belongs in `lib.rs` (imported by the binary with `use`), never duplicated.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'TRPL 7.2 privacy rule: an item is visible to its defining module *and all descendants*. `pub` on `inner` controls outside access to `inner` — it says nothing about what `inner` may see inward.',
        },
        {
        type: 'find_bug',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`self` in a nested list refers to the parent path itself. The glob story also compiles but imports everything — the book prefers explicit nested lists.',
        },
        {
        type: 'find_bug',
        ...
        correct: 'a',
        explain: 'TRPL 7.4: in a nested use-tree, `self` refers to the base path itself — needed when code uses both `io::stdin()`-style paths and the `Write` trait. The receiver-only story confuses path-`self` with receiver-`self`; the glob story would be `use std::io::*`.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 8 — Common Collections =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        options: [{ id: 'b', text: 'v.get(i)' }, { id: 'a', text: 'v[i]' }, { id: 'c', text: 'v.at(i)' }],
        correct: 'b',
        ...
        explain: 'Borrow violation (E0506): `first` immutably borrows `v` while `push` needs `&mut` — and `push` may reallocate, which would leave `first` dangling. TRPL 8.1: the checker conservatively assumes reallocation. Fix: print before pushing, or index after: `v.push(4); println!("{}", v[0]);`.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Why does `let s = s1 + &s2;` move `s1`, while `let s = format!("{s1}{s2}");` only borrows both?',
        options: [
          ...
          { id: 'a', text: '`+` desugars to `add(self, &str)` — it consumes the receiver by value; `format!` only borrows its arguments' },
        ],
        correct: 'a',
        explain: '`impl Add<&str> for String` takes `self` by value (TRPL 8.2), so `s1` moves. `format!` takes references throughout — which is why the book funnels string building toward `format!`/`push_str` whenever `s1` is still needed.',
        },
        {
        type: 'predict_output',
        prompt: 'What does this print?\n\nfn main() {\n    let s = "abc";\n    for b in s.bytes() {\n        print!("{b} ");\n    }\n}',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'What does `m.entry(key).or_insert_with(Vec::new)` return, and why does that matter?',
        options: [
          { id: 'b', text: 'A copy of the stored value' },
          { id: 'a', text: '`&mut V` — a live handle into the map, so you can `push` without a second lookup' },
          { id: 'c', text: '`Option<&mut V>` — `None` when the key already existed' },
          { id: 'd', text: 'The old value that was just replaced' },
        ],
        correct: 'a',
        explain: 'TRPL 8.3: the entry API returns `&mut V` to the live slot — one lookup for check-and-mutate is its whole point. The `Option<&mut V>` and old-value stories describe `insert`, which returns `Option<V>` (the displaced old value).',
        },
      ],
    },
  },
  ...
  // ================= Chapter 9 — Error Handling =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: '`expected` does substring matching against the panic payload. Bare `#[should_panic("msg")]` is not valid attribute syntax — the payload needs the `expected = ` key.',
        },
        {
        type: 'predict_output',
        ...
        options: [
          ...
          { id: 'a', text: 'When the panic message must document an invariant the reader needs — `unwrap` panics with a generic message that hides context' },
          { id: 'd', text: 'Only in tests — `expect` is rejected in binaries' },
        ],
        correct: 'a',
        explain: 'Both panic on `Err`; `expect` attaches your message to the panic output (TRPL 9.3/11.1). The never-panics and returns-`Option` stories invent non-panicking behavior, and the tests-only story invents a restriction. Book rule of thumb: `expect` where failure means programmer error with context worth stating.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Why does `?` compile here even though `parse` fails with `ParseIntError`, not `Box<dyn Error>`?\n\nfn f() -> Result<(), Box<dyn Error>> {\n    let n: i32 = "42".parse()?;\n    Ok(())\n}',
        options: [
          { id: 'a', text: '`?` converts via `From`: `ParseIntError` implements `Into<Box<dyn Error>>`, so the error is boxed automatically' },
        ...
        correct: 'a',
        explain: 'The `?` desugar calls `From::from` on the error (TRPL 9.2/12.3 `Box<dyn Error>` acceptance). Any `E: Error + \'static` converts into the box — that is why `main() -> Result<(), Box<dyn Error>>` absorbs every fallible call. The must-`map_err`-by-hand story is what beginners write before learning this.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 10 — Generics, Traits, Lifetimes =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'After monomorphization, what does the binary contain for `fn id<T>(x: T) -> T` used with `i32` and `String`?',
        options: [
          { id: 'b', text: 'One generic copy with runtime type tags' },
          { id: 'a', text: 'Two specialized copies — one per concrete type, each as fast as hand-written code' },
          { id: 'c', text: 'Nothing — generics are erased before codegen, as in Java type erasure' },
          { id: 'd', text: 'One copy that boxes both types behind a vtable' },
        ],
        correct: 'a',
        explain: 'TRPL 10.1: monomorphization stamps out a concrete copy per use — zero-cost at runtime, larger binary as the tradeoff. The erased-before-codegen story describes Java erasure, Rust\'s explicit anti-model; the boxes-behind-a-vtable story describes `dyn Trait` dispatch instead.',
        },
        {
        type: 'find_bug',
        prompt: 'struct Point<T> { x: T, y: T }\nfn main() {\n    let p = Point { x: 1, y: 2.0 };\n}\n\nWhy rejected?',
        explain: 'One `T` per struct instance: `x: i32` fixes `T = i32`, so `y: f64` mismatches (E0308). TRPL 10.1: mixed-type points need two parameters — `struct Point<T, U>`. The misconception is that each field gets its own `T`.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'What is the practical difference between `fn f(x: impl Display)` and `fn f<T: Display>(x: T)`?',
        options: [
          ...
          { id: 'a', text: 'None for a single parameter — `impl Trait` is sugar; differences appear with multiple params (each gets its own anonymous type parameter)' },
        ],
        correct: 'a',
        explain: 'TRPL 10.2: single-param `impl Trait` desugars to exactly the generic form (both monomorphize, both static). The difference: two `impl Trait` params are independent types, while one `T` used twice forces sameness. The dynamic-dispatch and no-monomorphization stories wrongly assign dynamism — that is `dyn Trait`.',
        },
        {
        type: 'multiple_choice',
        prompt: 'Given `trait Loud: Display { fn loud(&self); }` — what does the `: Display` supertrait bound require?',
        options: [
        ...
        correct: 'a',
        explain: 'TRPL 10.2/19.3: supertraits constrain implementors, not callers — `impl Loud for X` fails unless `X: Display` too. The auto-implements story inverts the direction; a trait never implements things on your behalf.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          { id: 'a', text: 'No — with two input lifetimes and no `&self`, elision cannot assign the output; annotate explicitly' },
        ...
        correct: 'a',
        explain: 'TRPL 10.3 elision: rule 1 gives `x` and `y` distinct lifetimes; rules 2–3 (single input / `&self` method) do not apply, so the output has no determinate source — E0106. Fix: `fn first<\'a>(x: &\'a str, y: &\'a str) -> &\'a str`. The always-first-input-lifetime story states a rule that does not exist.',
        },
        {
        type: 'multiple_choice',
        ...
        options: [
          ...
          { id: 'a', text: 'That `T` owns all its data (no borrowed content with shorter lifetimes) — NOT that values live for the whole program' },
          { id: 'd', text: 'That `T` implements `Drop` at program shutdown' },
        ],
        correct: 'a',
        explain: 'TRPL 10.3/16: `\'static` as a bound means "bounded by the end of the program" — owned types (`String`, `i32`) qualify; `&\'short str` does not. It constrains what the type may *borrow*, not how long any value lives.',
        },
        {
        type: 'find_bug',
        prompt: 'struct H {\n    s: &str,\n}\nfn main() {\n    let h = H { s: "hi" };\n}\n\nWhat is missing?',
        explain: 'Structs holding references need a lifetime parameter: `struct H<\'a> { s: &\'a str }` (TRPL 10.3). The compiler must record how long the borrow inside `H` stays valid — otherwise `H` could outlive its data. Same fix shape as function signatures, applied at the definition site.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 11 — Writing Automated Tests =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'What does `fn t() -> Result<(), String>` as a test buy you over `assert!` + `unwrap`?',
        options: [
          ...
          { id: 'a', text: '`?` inside the test — fallible helpers propagate with real error values instead of panicking blind' },
        ],
        correct: 'a',
        explain: 'TRPL 11.1: `Result`-returning tests let `?` propagate (`Err` = failure with the value attached). The cannot-return-values story denies a stable feature; `#[should_panic]` and `#[ignore]` cover the other special cases.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'find_bug',
        prompt: 'tests/auth.rs\nuse mycrate::helpers::login;\n\n#[test]\nfn can_login() { ... }\n\n`helpers` is declared `mod helpers;` (private) in `lib.rs`. Why does the integration test fail to build?',
        explain: 'Integration tests are separate crates linking only your *public* API (TRPL 11.3): private `mod helpers` is invisible outside `lib.rs` itself. Fix: `pub mod helpers;` — or move the helper to `tests/common/mod.rs`, the conventional shared-but-unpublished spot from the previous question.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 12 — I/O Project =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Why does the book refactor minigrep\'s config parsing onto iterators (`args.skip(2)`, `contents.lines()`) instead of indexing?',
        options: [
          { id: 'b', text: 'Iterators are always faster than indexing' },
          { id: 'c', text: 'Indexing `env::args()` is forbidden by the type system' },
          { id: 'a', text: 'Clarity plus boundary safety: no manual index bookkeeping means no off-by-one panics; the code states what it wants, not how to walk' },
          { id: 'd', text: 'It avoids all heap allocations' },
        ],
        correct: 'a',
        explain: 'TRPL 12.4/13: iterator adapters encode intent (`skip`, `lines`, `collect`) where indexing encodes mechanics — and every manual index is a potential out-of-bounds panic. The always-faster story overclaims; the win is correctness-by-construction.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 13 — Closures and Iterators =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Which statement about these closures is true?\n\nlet a = || println!("hi");\nlet mut v = vec![1];\nlet b = || v.push(2);\nlet s = String::from("x");\nlet c = || drop(s);',
        options: [
          { id: 'b', text: 'All three are `Fn` — captures never affect the trait' },
          { id: 'a', text: '`a: Fn`, `b: FnMut`, `c: FnOnce` — the compiler picks the least capable trait each body allows' },
          { id: 'c', text: '`c` is `FnMut` because `drop` mutates `s`' },
          { id: 'd', text: 'None implements any `Fn` trait without an explicit annotation' },
        ],
        correct: 'a',
        explain: 'A closure captures only what its body needs, as cheaply as possible. TRPL 13.1 capture ladder: a shared-borrow body → `Fn`; an `&mut` body → `FnMut`; by-value move-out (`drop(s)`) → `FnOnce`. The `drop`-mutates story misreads moving as mutating — `drop` consumes, it does not borrow mutably.',
        },
        {
        type: 'multiple_choice',
        prompt: 'Why must a function returning a closure use `impl Fn` (or a trait object) rather than naming the type?',
        options: [
          { id: 'a', text: 'Every closure has a unique anonymous compiler-generated type — there is no nameable type to write' },
        ...
        correct: 'a',
        explain: 'TRPL 13.1: closure types are unnameable anonymous structs, so `impl Trait` in return position (or `Box<dyn Fn()>`) is the only way to name "some closure". The bar-syntax story invents syntax; the cannot-return-at-all story is false — indirection solves sizing.',
        },
      ],
    },
  },
  {
    ...
    concept: 'Iterators and Adapters',
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Moved-from — `into_iter` consumes the vector; using `v` after is E0382' },
        ],
        correct: 'a',
        explain: '`into_iter(self)` takes ownership (TRPL 13.2) — the vector is consumed so iteration can hand out owned values. Borrowing iteration is `iter()`/`iter_mut()`. The only-borrows story describes `iter()`; the cloned-internally story invents a clone.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 14 — More About Cargo =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: '`serde` with `default-features = false, features = ["derive"]` — what does this buy?',
        options: [
          { id: 'b', text: 'It makes `serde` run faster at runtime' },
          { id: 'c', text: 'It pins `serde` to an exact version' },
          { id: 'a', text: 'A leaner dependency tree: opt out of the default feature set, opt into only `derive` — less code compiled, fewer transitive deps' },
          { id: 'd', text: 'Nothing — features are deprecated in favor of editions' },
        ],
        correct: 'a',
        explain: 'TRPL 14 (features): default features serve the common case; opting out trims compile time and dependency surface. Version pinning is the lockfile\'s job, not features\'.',
        },
      ],
    },
  },
  ...
  // ================= Chapter 15 — Smart Pointers =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'a',
        explain: 'The `Box` itself is a plain pointer (`Sized`) sitting wherever the variable lives — usually the stack — pointing at heap memory holding the `5` (TRPL 15.1). The heap-pointer story inverts the picture; the both-on-the-stack story would make `Box` pointless.',
        },
        {
        type: 'predict_output',
        prompt: 'What does this print?\n\nfn main() {\n    let b = Box::new(5);\n    println!("{}", *b + 1);\n}',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Why does the standard library hide `Drop::drop` behind `std::mem::drop` instead of letting you call `x.drop()`?',
        options: [
          { id: 'a', text: 'Explicit `x.drop()` would double-drop: the compiler still drops `x` at scope end, but `mem::drop(x)` moves ownership so no scope-end drop remains' },
        ...
        correct: 'a',
        explain: 'TRPL 15.3: `Drop::drop(&mut self)` only borrows — calling it would run cleanup early AND again at scope end (double free). `mem::drop(x)` takes ownership, moving the value so no scope-end drop remains. That is why the method is disallowed and the free function exists.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Reference cycles that leak — `Weak` does not count toward ownership, so cyclic structures can still deallocate' },
        ],
        correct: 'a',
        explain: 'TRPL 15.6: strong-count cycles (parent↔child `Rc`s) never reach zero and leak. `Weak` breaks the cycle by observing without owning (`upgrade()` to access). The cross-threads story describes `Arc`; the mutate-through-`Rc` story describes interior mutability.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Compile error or runtime panic — and where?\n\nfn main() {\n    use std::cell::RefCell;\n    let x = RefCell::new(5);\n    let a = x.borrow();\n    let b = x.borrow_mut();\n    println!("{} {}", a, b);\n}',
        options: [
          { id: 'b', text: 'Compile error at `borrow_mut` — the borrow checker sees the conflict' },
          { id: 'a', text: 'Compiles, then panics at `borrow_mut` — `RefCell` enforces borrowing rules at runtime while `a` is still live' },
          { id: 'c', text: 'Compiles and runs — `RefCell` allows one shared plus one mutable borrow together' },
          { id: 'd', text: 'Panics at `borrow`, the very first call' },
        ],
        correct: 'a',
        explain: 'TRPL 15.5: the whole point of `RefCell` is moving borrow checking to runtime — the compiler accepts what it would reject for plain references, and `borrow_mut` panics (`already borrowed`) while `a` lives. The compile-error story is the key misconception: with `RefCell`, the compiler deliberately looks away.',
        },
        {
        type: 'predict_output',
        ...
        explain: 'The `borrow_mut()` guard lives for the whole statement while `x.borrow()` requests a shared borrow inside it — already-mutably-borrowed panic. TRPL 15.5: guards release at statement/block end, not mid-expression. Fix: `let n = x.borrow().len(); x.borrow_mut().push(n);`.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Blocks until the thread finishes and propagates its `Result` — a child panic arrives as `Err` here' },
        ],
        correct: 'a',
        explain: 'TRPL 16.1: `join` blocks for completion; the `Result` carries the thread\'s return value or its panic payload. Dropping the handle does NOT stop the thread — only process exit does (the previous question\'s lesson).',
        },
        {
        type: 'multiple_choice',
        prompt: '`thread::spawn` requires its closure to be `Send`. Why does an `Rc<i32>` capture fail this bound while `Arc<i32>` passes?',
        options: [
          ...
          { id: 'a', text: '`Rc` counting is non-atomic — concurrent increments would race; `Arc` uses atomics and earns `Send` + `Sync`' },
          { id: 'd', text: '`Arc` clones the data per thread, `Rc` does not' },
        ],
        correct: 'a',
        explain: 'TRPL 16.1/16.4: `Rc<T>` is `!Send + !Sync` because plain-integer increments race across threads; `Arc` pays for atomic ops. The clones-per-thread story invents per-thread cloning — both pointers share one allocation.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          { id: 'a', text: 'Clone the `Sender` (`tx.clone()`) per producer — the channel is multi-producer, single-consumer' },
        ...
        correct: 'a',
        explain: 'TRPL 16.2: `mpsc` = multiple producer, single consumer — `Sender: Clone`, `Receiver: !Clone`. The `Arc<Mutex<..>>` wrapper works but fights the API; the book\'s pattern is cloned senders plus one receiver.',
        },
        {
        type: 'find_bug',
        prompt: 'fn main() {\n    use std::sync::mpsc;\n    let (tx, rx) = mpsc::channel();\n    let msg = String::from("hi");\n    tx.send(msg).unwrap();\n    println!("{msg}");\n}\n\nWhy rejected?',
        explain: '`send` takes ownership — the `String` moves into the channel (that IS the "share by communicating" transfer). `println!("{msg}")` borrows a moved value (E0382). Fix: `clone()` before sending, or stop using `msg` afterwards.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'A second thread locks a `Mutex` whose previous holder panicked while holding it. What happens?',
        options: [
          { id: 'b', text: 'It blocks forever' },
          { id: 'c', text: 'It silently succeeds with default data' },
          { id: 'a', text: 'The lock returns `Err(PoisonError)` — the mutex is "poisoned" to flag possibly-broken invariants; the locker decides whether to recover via `into_inner()`' },
          { id: 'd', text: 'The whole program aborts immediately' },
        ],
        correct: 'a',
        explain: 'TRPL 16.3: poisoning is a flag, not a deadlock — `lock()` yields `Err`, and `.unwrap()` on it is what most examples use (panicking in turn). Recovery is `poison_error.into_inner()`. The blocks-forever story confuses poisoning with deadlock; the book stresses poisoned ≠ locked-forever.',
        },
        {
        type: 'predict_output',
        ...
        explain: 'Self-deadlock: the second `lock()` blocks forever waiting on a guard the same thread holds — `Mutex` is not reentrant. Locks are runtime state with invisible ordering, so the compiler cannot see it; this compiles cleanly and hangs. Fix: scope the first guard (inner block or `drop(_a)`) before relocking.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'An executor (or an `.await` chain reaching one) — the block itself only builds a lazy state machine' },
          { id: 'c', text: 'The compiler runs it at compile time, like `const`' },
          { id: 'd', text: 'It runs immediately, line by line, like a closure body' },
        ],
        correct: 'a',
        explain: 'TRPL 17: `async` produces an inert future; nothing executes until an executor polls it. The fresh-OS-thread story confuses async with `thread::spawn`; the runs-immediately story confuses it with closures.',
        },
        {
        type: 'find_bug',
        prompt: 'async fn fetch() -> i32 { 42 }\nfn main() {\n    let x = fetch();\n    println!("{x}");\n}\n\nWhat goes wrong?',
        explain: '`fetch()` returns an un-awaited opaque future, not `i32` — and it implements no `Display`, so `{x}` fails (E0277). The classic beginner error: treating `async fn` as returning the value. Fix: drive it to completion (`block_on(fetch())`, or `.await` inside an async context) and print the resulting `i32`.',
        },
        {
        type: 'predict_output',
  ...
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          { id: 'a', text: 'It is cancelled — dropping a future stops its progress; destructors of live state still run' },
        ...
        correct: 'a',
        explain: 'TRPL 17: cancellation-by-dropping is the async model — unlike threads, no handle outlives the future, so `select!` losers simply drop. Scope-guard/`Drop` cleanup still executes, which is exactly why holding locks across `.await` is dangerous.',
        },
        {
        type: 'multiple_choice',
        ...
        options: [
          ...
          { id: 'a', text: 'Borrowed captures would force the future to borrow stack data that may not outlive the spawned task — `move` gives the future ownership' },
          { id: 'c', text: 'Without `move`, async blocks cannot contain `.await`' },
          { id: 'd', text: 'It is pure convention with no semantic effect' },
        ],
        correct: 'a',
        explain: 'TRPL 17: spawned tasks must be `\'static`; borrowing locals violates that. `move` transfers ownership into the future — the same rule as `thread::spawn`, applied to futures.',
        },
        {
        type: 'find_bug',
        prompt: 'async fn f() {\n    let data = vec![1, 2, 3];\n    tokio::spawn(async {\n        println!("{:?}", data);\n    });\n}\n\nWhy does spawning reject this?',
        explain: 'The inner future borrows `data` from the enclosing frame, but spawned tasks must be `\'static` — the task may outlive `f`. The borrow checker refuses (future is not `\'static`). Fix: `async move` so the future owns `data`.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'c', text: 'All struct fields are permanently private and reached via reflection' },
          { id: 'a', text: 'Privacy by default at module boundaries — fields stay hidden unless `pub`, and modules define the API surface' },
          { id: 'd', text: 'It does not — Rust has no encapsulation mechanism' },
        ],
        correct: 'a',
        explain: 'TRPL 18.1: encapsulation in Rust is a module-system property, not a class property. `pub` opens precisely what the API promises; everything else hides by default. The `encapsulated`-keyword story invents a keyword.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: '`fn clone(&self) -> Self;` — returns `Self`, whose size the vtable cannot know' },
        ],
        correct: 'a',
        explain: 'TRPL 18.2 object safety: methods returning `Self` (or generic methods) break `dyn` because the erased type\'s size is unknown. `&self`/`&mut self`/`Box<Self>` receivers are all object-safe. Escape hatch: `where Self: Sized` excludes the method from the vtable.',
        },
        {
        type: 'find_bug',
        prompt: 'trait T {}\nfn f(x: T) {}\nfn main() {}\n\nWhy rejected?',
        explain: 'A bare trait name is not a type: the compiler demands `dyn T`, and since `dyn` is unsized it must sit behind a pointer — `fn f(x: &dyn T)`. Every `dyn` needs indirection because its size is unknowable at compile time.',
        },
        {
        type: 'predict_output',
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Evaluates to `bool` — true when the value fits the pattern, without binding anything' },
        ],
        correct: 'a',
        explain: 'TRPL 19.1: `matches!(opt, Some(_))` is boolean pattern testing — `_` discards bindings. It shines in `assert!`/`filter` positions where a full `match` would be noise. The binds-like-`match` story is backwards: `matches!` deliberately binds nothing usable.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'In `match n { x if x > 10 => ..., 5 => ..., _ => ... }` with `n = 5` — which arm runs?',
        options: [
          { id: 'b', text: 'The guard arm — guards always win over literals' },
          { id: 'c', text: 'Neither — overlapping arms are a compile error' },
          { id: 'a', text: 'The `5` arm — guards check in order; the failing guard falls through to the literal' },
          { id: 'd', text: 'It panics — `x` is unbound in the guard arm' },
        ],
        correct: 'a',
        explain: 'TRPL 19.3: arms evaluate top-down; a failing guard falls through to later arms. Overlapping arms are legal (later ones are flagged unreachable only when provably shadowed). With `n = 5`: guard `5 > 10` fails → the literal matches.',
        },
        {
        type: 'predict_output',
  ...
  // ================= Chapter 20 — Advanced Features =================
  {
    ...
    checkpoint: {
      questions: [
        ...
        correct: 'b',
        explain: 'TRPL 20.1 lists exactly five: dereferencing raw pointers, calling `unsafe` fns, touching `static mut`, implementing unsafe traits, accessing `union` fields. Panic-catching (`catch_unwind`) is safe Rust. The `unsafe` keyword marks the rest as caller-audited.',
        },
        {
        type: 'find_bug',
        prompt: 'unsafe fn danger() {}\nfn main() {\n    danger();\n}\n\nWhy rejected?',
        explain: 'Calling an `unsafe fn` requires an `unsafe` block — the caller must opt into upholding its preconditions (TRPL 20.1). Fix: `unsafe { danger(); }`. Note the inverse is free: safe code runs fine inside `unsafe` blocks.',
        },
        {
        type: 'find_bug',
        prompt: 'fn main() {\n    unsafe {\n        println!("{}", abs(-3));\n    }\n}\n\nWhy won\'t this link, let alone run?',
        explain: '`abs` was never declared: calling C requires `unsafe extern "C" { fn abs(x: i32) -> i32; }` so Rust knows the symbol and its ABI (TRPL 20.1 FFI). `unsafe` alone cannot summon a foreign symbol — declaration first, `unsafe` call second.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'When each implementor has exactly one natural choice (e.g. `Iterator::Item`) — callers avoid annotating a parameter that can only ever be one thing' },
          { id: 'c', text: 'Associated types are always faster at runtime' },
          { id: 'd', text: 'Never — generics strictly dominate associated types' },
        ],
        correct: 'a',
        explain: 'TRPL 20.3: one-impl-one-choice → `associated type` (no annotation burden); many-choices-per-type → `generics` (an associated type would forbid a second impl). The many-simultaneous-choices story states the generic case; the always-faster story invents a runtime difference — both monomorphize identically.',
        },
        {
        type: 'find_bug',
        prompt: 'trait A { fn f(&self); }\ntrait B { fn f(&self); }\nstruct S;\nimpl A for S { fn f(&self) {} }\nimpl B for S { fn f(&self) {} }\nfn main() {\n    let s = S;\n    s.f();\n}\n\nWhy is the call ambiguous, and what is the fix?',
        explain: 'Both traits supply `f` — method resolution cannot choose (E0034). Disambiguate with fully-qualified syntax: `A::f(&s);` (TRPL 19.3/20.3 UFCS). The impls themselves are legal; only the dot-call is ambiguous.',
        },
        {
        type: 'multiple_choice',
        prompt: 'Why can `str` (unsized) only appear as `&str`, `Box<str>`, or behind another pointer?',
        options: [
          { id: 'a', text: 'Dynamically sized types have no compile-time-known size, so every use needs a pointer carrying the size (length/vtable) alongside' },
          { id: 'b', text: 'The compiler never implemented direct `str` variables' },
          { id: 'c', text: '`str` is always heap-allocated and must be boxed' },
          { id: 'd', text: 'Unsized types are forbidden entirely — even `&str` is legacy syntax' },
        ],
        correct: 'a',
        explain: 'TRPL 20.4 (DST): `str`/`[T]`/`dyn Trait` lack `Sized`; fat pointers (`&str` = address + length) restore knowability. The always-heap story is false — `&str` routinely borrows non-heap statics; the forbidden-entirely story denies the language\'s core string type.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: 'Why can a `macro_rules!` arm shadow an outer variable without breaking the caller?',
        options: [
          { id: 'a', text: 'Hygiene: declarative macros get fresh scopes for locals they introduce — caller bindings with the same name are untouched' },
        ...
        correct: 'a',
        explain: 'TRPL 20.5: `macro_rules!` hygiene keeps macro-introduced bindings separate from call-site bindings. Note the contrast with the previous question: re-running *expressions* (double evaluation) is a different hazard than capturing *names*.',
        },
        {
        type: 'find_bug',
        prompt: 'macro_rules! add {\n    ($a:expr, $b:expr) => { $a + $b };\n}\nfn main() {\n    println!("{}", add!(1));\n}\n\nWhy rejected?',
        explain: 'No arm matches a single expression — the only matcher demands two comma-separated `:expr` fragments (TRPL 20.5). Macro expansion matches textually against matchers before any code runs. Fix: `add!(1, 2)` — or add a `($a:expr) => { ... }` arm.',
        },
        {
        type: 'multiple_choice',
        ...
        options: [
          ...
          { id: 'a', text: '`#[derive(Serialize)]` → derive macro; `#[route]`-style wrappers → attribute macro; `sql!(...)` constructors → function-like macro' },
        ],
        correct: 'a',
        explain: 'TRPL 20.5: derive macros attach to structs/enums; attribute macros rewrite arbitrary `#[x]` items (arguments like `#[route(GET, path)]` allowed); function-like macros sit in call position. The no-arguments story denies real-world attribute args; the expressions-only story denies `sql!`-generated items.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'An iterator of `Result<TcpStream>` — each accepted connection (or accept error) arrives in turn' },
          { id: 'd', text: 'One `TcpStream` multiplexing every connection' },
        ],
        correct: 'a',
        explain: 'TRPL 21.1: `incoming()` is a blocking iterator over accept results — the `for stream in listener.incoming()` loop IS the single-threaded server. Each item is independently `Ok(stream)`/`Err`, which is why the book handles errors per item.',
        },
        {
        type: 'multiple_choice',
        prompt: 'Workers loop on `receiver.recv()`. Why does dropping the `Sender` terminate the loop instead of hanging?',
        options: [
          ...
          { id: 'a', text: '`recv()` returns `Err` on disconnect, ending `while let Ok(job)` loops — disconnection is the shutdown signal' },
        ],
        correct: 'a',
        explain: 'TRPL 21.2/21.3 graceful shutdown: no sentinel value or flag needed — channel disconnection IS the signal. Forgetting to drop the sender (holding a clone somewhere) is the classic hang; audit your `Sender` clones first.',
        },
        {
        type: 'multiple_choice',
        prompt: 'Why does the book call the final `ThreadPool` version "graceful" compared to thread-per-connection?',
        options: [
          { id: 'a', text: 'Bounded workers plus clean shutdown: no unbounded spawning under load, and `Drop` drains in-flight requests instead of abandoning them' },
        ...
        correct: 'a',
        explain: 'TRPL 21.2/21.3: unbounded spawning is a denial-of-service vector (slowloris-style); the pool caps concurrency, and `Drop` joins workers after disconnecting the queue so in-flight work completes. The faster-requests story confuses throughput architecture with latency.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Promotes all default clippy lints to hard errors — CI fails instead of warning' },
          { id: 'd', text: 'Only affects `cargo fmt`, not clippy' },
        ],
        correct: 'a',
        explain: 'Lint levels (`allow`/`warn`/`deny`/`forbid`, Appendix + TRPL App D): `deny` turns diagnostics into build failures. The book\'s posture: `warn` locally, `deny` in CI. The disables-clippy story describes `#![allow]`; levels never change what clippy *checks*, only how failures surface.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: '`try_into()` can fail, so each implementation names its failure type; infallible `From` needs no such channel' },
          { id: 'c', text: 'To make `TryFrom` slower and discourage its use' },
          { id: 'd', text: 'There is no difference — `From` also has an `Error` type' },
        ],
        correct: 'a',
        explain: 'RBE conversions + TRPL 9/10: `TryFrom::Error` carries the failure (e.g. `TryFromIntError`); `From` cannot fail by contract, so no channel exists. This is also why `?` works smoothly on `TryFrom` results while `From` needs no error plumbing at all.',
        },
        {
        type: 'multiple_choice',
        prompt: '`let s: String = "hi".into();` — why does this compile without naming `String::from`?',
        options: [
          { id: 'a', text: 'Blanket impl: `From<&str> for String` exists, and every `From` gives a free `Into` — inference picks the target from the annotation' },
        ...
        correct: 'a',
        explain: 'RBE conversions: `impl From<&str> for String` plus the std blanket `impl<T, U: From<T>> Into<U> for T`. The annotation selects which `From` applies — without it, `.into()` is genuinely ambiguous (the classic inference error). The always-`String` and ignored-annotation stories deny the annotation-driven resolution.',
        },
      ],
    },
  },
  {
    ...
    checkpoint: {
      questions: [
        ...
        {
        type: 'multiple_choice',
        prompt: '`#[derive(...)]` fails on your struct. Where do you check which traits are derivable?',
        options: [
          { id: 'b', text: 'Appendix A: Keywords' },
          { id: 'c', text: 'The `std::derive` module docs' },
          { id: 'a', text: 'Appendix C: Derivable Traits — the canonical list (`Debug`, `Clone`, `PartialEq`, …) with requirements' },
          { id: 'd', text: 'Nowhere — derivability is inferred automatically' },
        ],
        correct: 'a',
        explain: 'Appendix C is exactly the derivability index the book maintains. The `std::derive`-module story invents a module; the nowhere-to-look story denies the closed derive-macro set. Reference discipline: when the compiler names a missing trait, the appendix confirms whether `derive` can supply it.',
        },
      ],
    },
  },
    ...
    checkpoint: {
      questions: [
        ...
        options: [
          ...
          { id: 'a', text: 'Compile this item only on 64-bit Unix targets — `all` conjoins predicates' },
        ],
        correct: 'a',
        explain: 'RBE attributes: `all(...)` is conjunction (`any` disjoins, `not` negates). Like all of `#[cfg]`, it gates at compile time — the item does not exist in non-matching builds. The runtime-check story confuses `#[cfg]` with the `cfg!` macro; the enables-features story confuses platform predicates with Cargo features.',
        },
      ],
    },
  },
